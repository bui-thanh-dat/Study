...
-- dang 1 : so sanh bang (simple CASE)
SELECT fitst_name,
	CASE job_id
		WHEN 'IT_PROG' 		THEN 'Lap Trinh'
        WHEN 'FI_ACCOUNT' 	THEN 'Ke Toan'
        ELSE 'Khac'
	END AS bo_phan
FROM employees;

-- dang 2: dieu kien tu do (searched CASE)
SELECT first_name, salary,
	CASE 
		WHEN salary >= 15000 THEN 'Rat cao'
        WHEN salary >= 8000 THEN 'Cao'
        WHEN salary >= 5000 THEN 'Trung binh'
	END AS muc_luong
FROM employees
...
SELECT department_id,
		COUNT(*) AS tong_nv,
		SUM(CASE WHEN salary >= 8000 THEN 1 ELSE 0 END ) AS luong_cao,
        SUM(CASE WHEN salary < 8000 THEN 1 ELSE 0 END ) AS luong_thap,
        AVG(CASE WHEN job_id = 'IT_PROG' THEN salary END) AS luong_tb_it
FROM employees
